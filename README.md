...
- [x]  숫자 문자열 배열을 순회하며 각 요소를 정수로 변환하면서 양수인지 검증한다.
    - [x]  문자열이 비었으면 0으로 반환한다.
- [x]  커스텀 문자열이 `-`일 경우, `2-1`은 `[2, 1]`로, `2--1`을 `[2, -1]`로 처리한다.
...
- [x]  `"//"`와 `"\\n"` 중 하나만 존재하는 경우 `IllegalArgumentException`을 발생시키고 "`'//'와 '\\n'이 모두 존재해야 합니다.`" 라는 오류 메시지를 출력한 후
  애플리케이션을 종료한다.
- [x]  숫자 문자열 배열에 숫자가 아닌 값이 포함되어 있는 경우 `IllegalArgumentException`을 발생시키고 **"**`숫자가 아닌 값이 포함되어 있습니다.`**"** 라는 오류 메시지와 오류가
  발생한 값을 출력한 후 애플리케이션을 종료한다.
- [x]  숫자 문자열 배열에 숫자 문자열이 `long`으로 표현할 수 없는 숫자일 경우 `IllegalArgumentException`을 발생시키고 **"**`숫자가 너무 큽니다.`**"** 라는 오류 메시지와
  오류가 발생한 값을 출력한 후 애플리케이션을 종료한다.
- [x]  문자열에 음수 값이 포함된 경우 `IllegalArgumentException`을 발생시키고 **"**`음수는 허용되지 않습니다.`**"** 라는 오류 메시지와 오류가 발생한 값을 출력한 후
  애플리케이션을 종료한다.
- [x] 숫자를 합하는 중 합이 long의 범위를 넘어간 경우 IllegalArgumentException을 발생시키고 "합계가 long의 범위를 벗어났습니다."라는 오류 메시지를 출력한 후 애플리케이션을
  종료한다.
...
- [x]  MVC 패턴 적용
- [x]  ~~TDD 개발 방법론 준수~~ 테스트 커버리지 90%이상 달성
