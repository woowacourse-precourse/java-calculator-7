...
### 구현한 기능 목록
    ...
    3. 입력 문자열을 검증한다.
      ...
      - 만약 위의 경우가 아니면 IllegalArgumentException 발생
    4. 입력된 문자열에 커스텀 구분자 추출
      - 추출 후 커스텀 구분자 지정을 위한 부분을 삭제한다.
      - 커스텀 구분자를 구분자에 추가하라.(보류)
      - 커스텀 구분자에 \이 들어가면?
      - 커스텀 구분자가 한 글자가 아니면?
      - 커스텀 구분자가 숫자이면?
    5. 입력이 양수로 시작하면 기본 구분자에 대해 유효한지 확인한다.
      - 커스텀 구분자가 없음
    6. 입력이 커스텀 구분자로 시작하면 커스텀 구분자를 추가하고 유효한지 확인하다.
      - 이스케이프 문자와 정규표현식 메타문자의 경우 추가적인 처리가 필요하다.


### 구현할 기능 목록
    7. 사용자 입출력과 테스트 코트를 통한 커스텀 구분자 입출력 케이스에 차이가 있음
