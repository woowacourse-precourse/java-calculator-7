...
- [x] **사용자로부터 문자열을 입력받는다.**
    ...
    - [x] 입력된 문자열이 빈 문자열(`""`)인지 확인한다.
        - [x] **빈 문자열인 경우 결과로 `0`을 출력하고 다시 입력받는다.**
    - [x] **`null`도 아니고 빈 문자열도 아닌 경우 다음 단계를 진행한다.**
...
- [x] **커스텀 구분자가 있는지 확인한다.**
    - [x] 입력 문자열에서 `"//"`와 `"\n"`의 위치를 찾는다.
        - [x] **case 1: `"//"`와 `"\n"`이 모두 존재하고, `"//"`가 `"\n"`보다 앞에 위치하는 경우**
            - [x] `"//"`와 `"\n"` 사이의 문자를 **커스텀 구분자**로 추출한다.
                - [x] **커스텀 구분자가 비어 있는 경우**
                    - [x] `IllegalArgumentException`을 발생시키고 **"`커스텀 구분자가 비어 있습니다.`"** 라는 오류 메시지를 출력한 후 프로그램을 종료한다.
                - [x] **커스텀 구분자의 길이가 1이 아닌 경우**
                    - [x] `IllegalArgumentException`을 발생시키고 **"`커스텀 구분자는 문자 1개로 구성되어야 합니다.`"** 라는 오류 메시지를 출력한 후 프로그램을
                      종료한다.
                - [x] **커스텀 구분자가 숫자인 경우**:
                    - [x] `IllegalArgumentException`을 발생시키고 **"`커스텀 구분자는 숫자가 될 수 없습니다.`"** 라는 오류 메시지를 출력한 후 프로그램을 종료한다.
                - [x] **커스텀 구분자가 기본 구분자인 경우 (, 또는 :)**
                    - [x] `IllegalArgumentException`을 발생시키고 **"`커스텀 구분자는 기본 구분자(, 또는 :)로 설정할 수 없습니다.`"** 라는 오류 메시지를 출력한
                      후 프로그램을 종료한다.
            - [x] **커스텀 구분자와 기본 구분자(`,`, `:`)를 구분자 목록에 추가한다.**
            - [x] `"//"`와 `"\n"` 및 그 사이의 **구분자를 제외한 나머지 부분을 연산할 문자열**로 설정한다.
        - [x] **case 2: `"//"`와 `"\n"`이 모두 존재하지만, `"//"`가 `"\n"`보다 뒤에 위치하는 경우**
            - [x] `IllegalArgumentException`을 발생시키고 **"`'//'는 '\n'보다 앞에 위치해야 합니다.`"** 라는 오류 메시지를 출력한 후 프로그램을 종료한다.
        - [x] **case 3: `"//"`와 `"\n"` 중 하나만 존재하는 경우**
            - [x] `IllegalArgumentException`을 발생시키고 **"`'//'와 '\n'이 모두 존재해야 합니다.`"** 라는 오류 메시지를 출력한 후 프로그램을 종료한다.
        - [x] **case 4: `"//"`와 `"\n"`이 모두 존재하지 않는 경우**
            - [x] **기본 구분자(`,`, `:`)만을 구분자 목록에 추가한다.**
            - [x] **전체 입력 문자열**을 **연산할 문자열**로 설정한다.

### 문자열을 구분자로 나눈 후, 각 부분을 숫자로 변환하면서 검증

- [x] 구분자 목록을 사용하여 연산할 문자열을 분리하여 **숫자 문자열 배열을 생성한다.**


- [x] 숫자 문자열 배열을 순회하며 **각 요소를 정수로 변환하고 검증한다.**
    - [x] 변환 과정에서 **숫자가 아닌 값**이 있는 경우
        - [x] `IllegalArgumentException`을 발생시키고 **"`숫자가 아닌 값이 포함되어 있습니다`"** 라는 오류 메시지를 출력한 후 프로그램을 종료한다.
    - [x] 변환된 정수가 **음수 값**인 경우
        - [x] `IllegalArgumentException`을 발생시키고 **"`음수는 허용되지 않습니다`"** 라는 오류 메시지를 출력한 후 프로그램을 종료한다.
...
- 단일 구분자를 표현하는 객체로, 구분자를 정규식 패턴으로 변환하는 기능 제공
...
- 구분자를 사용하여 연산할 표현식을 분리된 문자열 리스트로 변환
...
- 계산을 수행하는 인터페이스로, 구체적인 계산 로직을 다양한 구현체에서 정의
...
- `"//"`와 `"\n"`의 위치를 확인하여 올바른 순서와 형식으로 입력되었는지 검사

#### PositiveIntegerValidator

- 문자열 리스트에 포함된 값들이 유효한 양의 정수인지 검사
...
- IllegalArgumentException을 상속받고, ErrorMessage를 인자로 받아 처리
