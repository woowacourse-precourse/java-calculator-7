...
- 계산기
    ...
    - [x] 분리한 숫자들로 리스트를 구성한다.
      - [x] `""` 일 경우 `0`으로 처리
    - [x] 숫자 리스트의 합을 구한다.
    - [x] 구한 덧셈 결과를 컴퓨터에게 반환한다.
    - [ ] 특수 기호를 구분자로 사용한다.
       - [x] 백슬래시 `\`를 구분자로 사용할 경우 해당 기호 뒤에 `\\`를 추가한다.
       - [x] 괄호 `(, )`나 캐럿 `^`을 구분자로 사용할 경우 해당 기호 앞에 `\\`를 추가한다.
...
- 출력
    - 컴퓨터
        ...
        - [x] 덧셈 결과 `"결과 : {덧셈 결과}"` 문구 출력

### 예외 처리 구현 리스트

- 사용자 입력 문자열
    - [x] 분리된 문자열이 숫자가 아닐 경우 예외 처리 `"[ERROR] 문자열은 구분자와 숫자만 입력되어야 합니다."`
    - [x] 분리된 문자열이 양수가 아닐 경우 예외 처리 `"[ERROR] 문자열은 구분자와 양수만 입력되어야 합니다."`
    - [x] 기본 구분자나 커스텀 구분자가 아닌 다른 입력을 사용했을 경우 예외 처리 `"[ERROR] 기본 구분자(콜론 ':', 쉼표 ',') 또는 커스텀 구분자('//구분자\n')를 사용해야 합니다."`
    - [x] 문자열 앞부분에 커스텀 구분자가 존재하지 않을 경우 예외 처리 `"[ERROR] 커스텀 구분자는 문자열 앞부분에 위치해야 합니다."`