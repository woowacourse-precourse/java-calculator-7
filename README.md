...
``` 
    ...
    ✅ 구현 후 느낀점 / 추가 개선사항 
    ㄴ 총 2개의 예외를 처리하는 Static Method를 작성하고 각각의 상황에 맞도록 DTO의 생성자와 Sevice의 연산하는 부분에 추가했다.
        ㄴ 예외 처리라는 것은 한번에 처리하는 것보다 상황에 필요한 예외를 부여하는 것이 좋다고 생각했다. 
        ㄴ 하지만 뭔가 이렇게 작은 서비스거나, 입력값에 따라 모든것이 결정된다면 통합 예외 테스트를 만들어서 처리하는 것도 나쁘지않아 보인다.
    ㄴ 이번 예외 처리 또한 각각의 역할 Ex_다음 값이 올바른 입력 요구사항을 만족하는가?와 사용자가 어떤 입력을 하면 문제가 발생하는가에 집중해보았다.
