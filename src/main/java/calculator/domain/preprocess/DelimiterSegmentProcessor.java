...
public class DelimiterSegmentProcessor {

    public boolean checkCustomDelimiterRequest(String input) {
        if (input.startsWith("//") && input.contains("\n")) {
            validateCustomDelimiterRequest(input);
            return true;
        } else
            return false;
    }
    ...
    private void validateCustomDelimiterRequest(String input) {
        if (input.indexOf("\n") == 2)
    ...
    public List<String> extractDelimiterList(String input) {
