...
public class DelimeterCheck {
    ...
    public String getSplitFormula(String formula) {
        ...
        if (formula.startsWith("//") && (formula.charAt(3)=='\\') && (formula.charAt(4)=='n')) {
            ...
            getCustomDelimeter(formula);
        } else {
            System.out.println("조건 불충족");
        }

        // 분리된 문자열을 splitFormula[] 배열로 저장
        String [] splitFormulaArray = formula.split(delimeter);
    ...
    // getSplitFormula에서 생성된 숫자 문자열 배열을 int로 변환
    // 변환 이후
    //    public int getSplitNumbers() {
    // 커스텀 구분자 저장, 최신화
    public String getCustomDelimeter(String formula) {
