...
public class CalculatorController {
    ...
    public int run(String input) {
        String processedInput = separatorService.extractAndAddSeparator(input);        // 커스텀 구분자 추출
        List<String> separators = separator.getSeparators();                           // 구분자 가져오기

        List<String> stringInputs =
                splitterService.split(processedInput, separators);   // 구분자로 문자열 분리
        List<Integer> numberInputs =
                converterService.toNumbers(stringInputs);           // 분리된 문자열 숫자로 변환

        return calculatorService.sum(numberInputs);                                    // 숫자 합 계산
    }
}
