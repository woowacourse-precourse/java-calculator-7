...
class DelimiterNumberHandlerTest {

    @DisplayName("입력값의 가장 처음이 구분자이면, 예외를 반환하는 메서드를 테스트하라")
    @Test
    void test_validateStart() {
        DelimiterNumberHandler delimiterNumberHandler = new DelimiterNumberHandler(":|,", ":1:23:3");

        assertThatThrownBy(delimiterNumberHandler::validateStart)
    ...
    void test_splitNumbers_with_error() {
        ...
        assertThatThrownBy(delimiterNumberHandler::splitNumbers)
                .isInstanceOf(IllegalArgumentException.class)
    ...
    void test_splitNumbers() {
        DelimiterNumberHandler delimiterNumberHandler = new DelimiterNumberHandler(":|,", "1:2:3");

        String[] numbers = delimiterNumberHandler.splitNumbers();
