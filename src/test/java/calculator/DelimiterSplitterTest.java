...
class DelimiterSplitterTest {
    ...
    void 기본구분자_분리_성공() {
        ...
        // when
        List<Integer> result = delimiterSplitter.splitByDelimiter().stream()
                .map(Number::getValue)
                .toList();
    ...
    void 숫자아닌문자_포함된_경우_실패() {
        ...
        // when & then
        assertThrows(IllegalArgumentException.class, delimiterSplitter::splitByDelimiter);
    }
    ...
    void 커스텀_구분자가_숫자여도_성공(){
        ...
        DelimiterSplitter delimiterSplitter = new DelimiterSplitter(target, List.of(new Delimiter("1"), new Delimiter(":")));
